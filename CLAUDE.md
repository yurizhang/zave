...
- `packaging/` — `package.sh` (universal `.app` + `.zip`/`.dmg`), `Info.plist`,
  `icon.icns`. Icon source lives in `design/zave-icon.svg` (+ a rendered
  `design/zave-icon.png`). Regenerate the icns:
  `qlmanage -t -s 1024 -o /tmp design/zave-icon.svg` → `sips` resize into an
  `.iconset` → `iconutil -c icns -o packaging/icon.icns`.
